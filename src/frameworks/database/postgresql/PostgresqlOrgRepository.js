module.exports = class PostgresqlOrgRepository {
  constructor(pool) {
    this.pool = pool;
  }
  ...
  async getOwnerId() {
    try {
      const selOrgRow = this.pool.query(`SELECT owner FROM organizations WHERE org_id = $1`, [req.params.orgId]);
  ...
  async getOrgList(page, city, category) {
    try {
      ...
      else sqlVar.page = (page - 1) * 10;
      if (city === undefined) sqlVar.city = '%';
      else sqlVar.city = city;
      if (category === undefined) sqlVar.category = '{0, 1, 2}';
      else sqlVar.category = "{" + category + "}";
      const orgRow = await this.pool.query(`
        SELECT 
        org.org_id AS id, 
        org.name AS name, 
  ...
  async editOrg(org, ownerId, avatar) {
    try {
      ...
      await pool.query(orgInsertString, [org.name, org.about, org.category, avatar, org.city, ownerId, org.orgId]);
      return true;
    } catch (err) {
      console.log(err);
      throw err;
  ...
  async getOrgQuestList(orgId, page = 1, status = '0, 1, 2, 3, 4, 5', paid = 'true, false') {
    try {
      ...
      return { quests: quRow.rows, count: count.rows.count }
    } catch (err) {
      console.log(err);
      throw err;
    }
